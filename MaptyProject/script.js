...
class App {
  ...
  _getPosition() {
    //getCurrentPosition(callback function when the browser gets the current location,2nd for an error)
  ...
  _newWorkout(e) {
    ...
    const validInputs = (...inputs) =>
      inputs.every(inp => Number.isFinite(inp)); // every will return true only if all parameters are
    //Get data from form
    ...
    // If workout cycling create cycliing object
  ...
  removeOne() {
    localStorage.removeItem(this.#workouts[0]);
    location.reload();
  }
  reset() {
